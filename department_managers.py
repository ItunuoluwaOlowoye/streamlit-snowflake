...
st.set_page_config(page_title='BuyMart Departments Portal', page_icon=browser_tab_logo, layout='wide') # set the page layout
...
if functions.authenticate_user(placeholder,sb_placeholder):
    if st.session_state.user.groups.filter(name__in=["Department Managers"]).exists(): # after authentication and confirming that user is a service dept head
        ...
        try:
            ...
            dept_database = full_database.query(f'region == "{region}" & dept == "{dept}"') # filter to service dept data
            dept_db = dept_database.loc[:,columns] # select relevant columns
            unpivot_dates_df = functions.arrange_dates(dept_database,columns,date_column,date_comment_column) # convert date columns to one column in olap format
        ...
        if report_type=='View tables': # for tables
            ...
            with full_table: # for full table
                interactive_table, full_modified_df = functions.edit_table(full_database, dept_db, date_column, editable_columns=[date_comment_column,'email_address','phone_number']) # create interactive table
                if full_modified_df.empty is False: # if updates are made
                    ...
                    if save: # to save
                        ...
                        functions.save_data_updates(full_modified_df,date_column,group_logs=f'{dept_manager}') # save updates
                        functions.update_db(group_logs=f'{dept_manager}') # update table
                        st.cache_data.clear() # clear cache
                        st.experimental_rerun() # rerun app
            with unaccounted_list:
                functions.filtered_people_list(dept_db, full_date, date_column, type='unaccounted', cols=['full_name','phone_number']) # list people who have neither been marked present nor absent
            with absent_list:
                functions.filtered_people_list(dept_db, full_date, date_column, type='absent', cols=['full_name','phone_number']) # list people who have been marked absent
        elif report_type=='See specific attendance report':
            st.header(f'Report: {full_date}')
            dept_or_branch_numbers, total_members, last_week_full_date, todays_total_attendance, todays_present_attendance, todays_present_attendance_percent, last_week_total_attendance, last_week_present_attendance, last_week_present_attendance_percent = functions.specific_date_summary_stats(unpivot_dates_df,attendance_date,dept_or_branch=None) # calculate summary stats; dataframe only useful for checkin
            ...
            dashboard_tab,dedicated_tab,inprogress_tab,icu_tab = functions.timeseries_trends(unpivot_dates_df, columns, facet_by='full_name',tab_name='dept') # create time series trends
    else:
